...
const AccommodationApproval = ()=>{
    ...
    const handleApprovalChange = async (e) => {
      ...
      try {
          const response = await axios.post(
              `/api/v1/booking/accommodationApproval?bookingId=${bookingId}`, 
              { approved }, 
    ...
    <div className="max-w-md mx-auto bg-white shadow-md rounded-lg p-6 mt-5">
      ...
      <div className="border-b pb-2 mb-2">
        <p><strong>Travel Agency:</strong> {booking?.accommodation?.name || "N/A"}</p>
      </div>
      <div className="border-b pb-2 mb-2">
        ...
        <p><strong>Accommodation Days:</strong>{booking?.travelDetails?.accommodation_days || 0} days</p>
      </div>
...
export default AccommodationApproval